...
fetch('https://api.nytimes.com/svc/news/v3/content/all/all.json?api-key=EW6VIEkH86qFPuhG7gGWGEWDMPGXjmzm')
  ...
  .then((json) => {
    ...
    <a href="${json.results[0].url}" class="text-wrapper" target="_blank"><h2><span>Right now:</span> ${json.results[0].title}</h2>
    <h3>${json.results[0].abstract}</h3></a>
    `

    json.results.slice(1).forEach((results) => {
      gridNews.innerHTML += `
      <section class="grid-wrapper"><img class="grid-image" src="${results.multimedia[2].url}" alt="Picture from latest news">
      <a href="${results.url}" class="grid-text-wrapper" target="_blank"><h4>${results.title}</h4>
      <h5>${results.abstract}</h5></a></section>
    `
    })
